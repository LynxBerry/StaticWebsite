...
function wordItemClass(status: StatusType) {
  ...
  switch (status) {
    ...
    case 'unlearned':
      ...
      // icon's bg-white/15 + the muted plant 🌰 already convey "unlearned";
...
const inputBaseClass =
  'flex-1 min-w-0 px-4 h-11 rounded-lg bg-white/15 backdrop-blur-md text-white text-sm outline-none ' +
  'transition-all duration-sprout-mid placeholder:text-white/45 ' +
  'focus:bg-white/25 focus:shadow-[0_0_0_2px_rgba(255,255,255,0.25)]';
        ...
        {showBatchForm && (
          <div className="glass-card mb-4 p-3 animate-toast-in">
            <p className="text-xs text-white/70 mb-2 leading-relaxed">
              每行一个单词，格式：<span className="text-white font-semibold">英文,中文</span>（逗号分隔，支持中文逗号）
            </p>
            <textarea
              className="w-full px-3 py-2 mb-3 rounded-xl bg-white/15 backdrop-blur-md text-white text-sm outline-none transition-all duration-sprout-mid placeholder:text-white/45 focus:bg-white/25 focus:shadow-[0_0_0_2px_rgba(255,255,255,0.25)] min-h-[120px] resize-y"
    ...
    <section className="flex-1 flex flex-col min-h-[60vh]" id="bank-view">
      <div className="mb-4">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl font-semibold font-display text-white">词库</h2>
        ...
        <input
          type="text"
          className="w-full mb-4 px-4 h-11 rounded-lg bg-white/15 backdrop-blur-xl backdrop-saturate-150 text-white text-sm outline-none transition-all duration-sprout-mid placeholder:text-white/45 focus:bg-white/25 focus:shadow-[0_0_0_2px_rgba(255,255,255,0.25)]"
          placeholder="搜索英文或中文..."
          value={searchTerm}
        ...
        {feedback && (
          <p className="text-xs text-white/70 text-center mb-2 animate-toast-in">{feedback}</p>
        )}
      </div>
                  ...
                  <div className="flex flex-col items-end gap-1 text-xs shrink-0">
                    ...
                    <div className="flex gap-1 mt-1">
                      <button
                        ...
                        className="w-7 h-7 flex items-center justify-center rounded-full bg-black/55 backdrop-blur-sm text-[0.625rem] hover:bg-black/75 transition-colors"
                        title="编辑"
                      >
                        ✏️
                      </button>
                      <button
                        ...
                        className="w-7 h-7 flex items-center justify-center rounded-full bg-black/55 backdrop-blur-sm text-[0.625rem] hover:bg-red-900/60 transition-colors"
                        title="删除"
                      >
                        🗑️
                      </button>
