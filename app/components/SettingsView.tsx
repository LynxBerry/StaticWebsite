...
const inputClass =
  'w-full px-4 py-2.5 rounded-lg bg-black/10 backdrop-blur-xl backdrop-saturate-150 text-black text-[0.9375rem] outline-none transition-all duration-200 placeholder:text-black/45 focus:bg-black/15 focus:shadow-[0_0_0_2px_rgba(0,0,0,0.15)]';
...
export default function SettingsView({ exportState, importState, onReset, siteTitle, onUpdateSiteTitle, dailyNewLimit, onUpdateDailyNewLimit }: SettingsViewProps) {
  ...
  return (
    <section className="flex-1 flex flex-col min-h-[60vh]" id="settings-view">
      <div className="mb-4">
        <h2 className="text-xl font-semibold text-black mb-1 font-display">⚙️ 设置</h2>
        <p className="text-sm text-black/60">备份、恢复和重置</p>
      </div>

      <div className={sectionClass}>
        <h3 className="text-base font-semibold text-black mb-2 font-display">🏷️ 站点名称</h3>
        ...
        />
        {titleSaved && (
          <p className="mt-3 px-4 py-1.5 rounded-full text-sm text-center bg-black/10 text-sprout-700 border border-black/10">
            已保存 ✓
          </p>
        )}
      </div>
      ...
      <div className={sectionClass}>
        <h3 className="text-base font-semibold text-black mb-2 font-display">📚 每日新词限额</h3>
        <p className="text-sm text-black/70 mb-4 leading-relaxed">
      ...
      <div className={sectionClass}>
        <h3 className="text-base font-semibold text-black mb-2 font-display">💾 备份词库和进度</h3>
        <p className="text-sm text-black/70 mb-4 leading-relaxed">
      ...
      <div className={sectionClass}>
        <h3 className="text-base font-semibold text-black mb-2 font-display">📂 恢复备份</h3>
        <p className="text-sm text-black/70 mb-4 leading-relaxed">
          选择之前导出的备份文件，恢复你的词库和学习进度。适用于换设备或重装后找回数据。
        </p>
        <label className="flex items-center gap-2 mb-4 text-sm text-black/90 cursor-pointer select-none">
          <input
            type="checkbox"
            className="w-[1.125rem] h-[1.125rem] accent-green-600 cursor-pointer"
            checked={mergeImport}
        ...
        {importMessage && <p className="mt-3 px-4 py-1.5 rounded-full text-sm text-center bg-black/10 text-sprout-700 border border-black/10">{importMessage}</p>}
      </div>

      <div className={sectionClass}>
        <h3 className="text-base font-semibold text-red-600/80 mb-2 font-display">🗑️ 重置进度</h3>
        <p className="text-sm text-black/70 mb-4 leading-relaxed">清空所有学习进度（不会删除词库）。此操作不可恢复，建议先导出备份。</p>
      ...
      <div className={sectionClass}>
        <h3 className="text-base font-semibold text-black mb-2 font-display">🔑 修改密码</h3>
        <p className="text-sm text-black/70 mb-4 leading-relaxed">
          修改你的登录密码。需要先验证旧密码。
        </p>
        <div className="space-y-3">
          ...
          />
          {passwordMessage && (
            <p className={`text-sm ${passwordMessage.type === 'success' ? 'text-sprout-700' : 'text-red-500'}`}>
      ...
      <div className={sectionClass}>
        <h3 className="text-base font-semibold text-black mb-2 font-display">🚪 退出登录</h3>
        <p className="text-sm text-black/70 mb-4 leading-relaxed">退出当前账号，返回登录页。你的学习进度会保留在这个设备的本账号下。</p>
