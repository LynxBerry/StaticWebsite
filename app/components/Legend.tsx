...
export default function Legend() {
  ...
  return (
    <section className="mb-6 p-4 glass-card">
      <div className="text-sm text-black/80 mb-4 font-semibold">熟悉度成长图</div>
      <div className="flex items-center justify-start gap-0.5 overflow-x-auto pb-2 -mb-2 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
        {steps.map((step, index) => (
          <div key={step.box} className="flex items-center gap-0.5 shrink-0">
            <div className={`flex flex-col items-center min-w-[3rem] px-0.5 py-1 rounded-lg transition-colors duration-200 ${step.mastered ? 'bg-black/10' : ''}`}>
              <span className="text-lg mb-0.5">{step.icon}</span>
              <span className="text-[0.75rem] font-bold text-black/90">{step.box}</span>
              <span className="text-[0.6875rem] text-black/70 whitespace-nowrap">{step.desc}</span>
            </div>
            {index < steps.length - 1 && <div className="text-black/40 text-xs font-bold opacity-80 shrink-0">→</div>}
          </div>
        ))}
      </div>
    </section>
